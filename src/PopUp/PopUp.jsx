...
export default function PopUp({ blurState, setBlurState, setData, isLoading, setIsLoading, isDescription = false,description=null}) {
  return (
    <div className={`w-1/4 h-[40%] shadow-2xl absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white z-20 rounded-xl`}
      ...
      {isDescription ?
        ...
        (<div className='w-full h-full flex flex-col items-center'>
          ...
          <textarea className='w-[90%] border h-10/12 -mt-2 px-2 py-1 text-md font-ubuntu font-semibold'>{description}</textarea>
        </div>)
      }
    </div>
  )
